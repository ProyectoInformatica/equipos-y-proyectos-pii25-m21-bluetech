...
def mostrar_pantalla_menu_admin(page: ft.Page, repo, usuario):
    #Imports diferidos para evitar recursividad
    ...
    def reconstruir_mapa_dashboard():
        ...
        for p in range(num_plantas):
            ...
            contenedor_mapa_vivo.controls.append(ft.Column([fila_sup, pasillo, fila_inf], horizontal_alignment=ft.CrossAxisAlignment.CENTER))
        page.update()
        ...
        content=ft.Column([
            ft.Row([
                ...
                ft.Column([
                    ...
                    ft.Text("Administrador", size=12, color="grey600"),
                ], spacing=1)
            ], spacing=10),
            
            ft.Divider(height=40, color="transparent"),
            
            ft.Text("MONITORIZACIÓN", size=11, color="grey500", weight="bold"),
            ft.ListTile(leading=ft.Icon(ft.Icons.DASHBOARD_ROUNDED, color="blue"), title=ft.Text("Dashboard"), selected=True),
            ft.ListTile(
                leading=ft.Icon(ft.Icons.MAP_OUTLINED), title=ft.Text("Mapa Interactivo"),
                on_click=lambda _: navegar(lambda: mostrar_pantalla_mapa_admin(page, repo, usuario))
            ),
            ft.ListTile(
            ...
            ft.ListTile(
                ...
                on_click=lambda _: navegar(lambda: mostrar_pantalla_gestion_usuarios(page, usuario, repo, lambda: mostrar_pantalla_menu_admin(page, repo, usuario)))
            ),
            ft.ListTile(
                leading=ft.Icon(ft.Icons.HEALTH_AND_SAFETY_OUTLINED), title=ft.Text("Parámetros Sanidad"),
                on_click=lambda _: navegar(lambda: mostrar_pantalla_parametros_sanidad(page, repo, usuario))
            ),
            ft.ListTile(
                leading=ft.Icon(ft.Icons.BOLT_OUTLINED), title=ft.Text("Consumo Energético"),
                on_click=lambda _: navegar(lambda: mostrar_pantalla_consumo_sensores(page, usuario, lambda: mostrar_pantalla_menu_admin(page, repo, usuario)))
            ),
            ft.ListTile(
    ...
    #--- PANEL DERECHO ---
    tarjeta_mapa_real = ft.Container(
