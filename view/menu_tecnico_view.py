...
def mostrar_pantalla_menu_tecnico(page: ft.Page, repo, usuario):
    ...
    tarjeta_inferior = ft.Container(
        height=250, bgcolor="white", border_radius=15, padding=25,
        shadow=ft.BoxShadow(blur_radius=10, color="black12"),
        content=ft.Column([
            ft.Text("Monthly Energy Costs", size=18, weight="bold"),
            ft.Text("Análisis de consumo mensual", size=12, color="grey600"),
            ft.Container(expand=True, content=ft.Text("Gráfico de barras aquí...", color="grey300"), alignment=ft.Alignment(0, 0))
        ])
    )
