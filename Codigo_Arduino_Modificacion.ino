...
void setup() {
  ...
  inicializarSensor(16, 17);
  Serial.println("Sensor biordinario inicializado");
...
void loop() {
  ...
  //Comprobamos los datos del nuevo sensor

  if (comprobarDatosDisponibles()){
    //tecnincamente se tendria que leer los  datos del sensor

    String datos[2];
    datos = leerDatosOrdinarios()

    //nos da unos datos
    //un int y un char
    //y lo guardamos en un array

    int valorNumerico = datos[0].toInt();
    String valorTexto = datos[1];
  }
  ...
  //Se utiliza la funcion para insertar a la base de datos
  void DatabaseInsert(
  int sensorId,
  int valorNumerico,
  String valorTexto
...
//---------
  ...
  //Aqui lo mostramos en serial los datos
